...
- When calling this tool, you MUST pass a summary of the research question for each section (fundamental, macro, quant) as a string.
  - `fundamental_q`: The question or guidance for fundamental analysis.
  - `macro_q`: The question or guidance for macro analysis.
  - `quant_q`: The question or guidance for quant analysis.
- Example tool call:
```
run_all_specialists_parallel(
  fundamental_q="Analyze the competitive landscape of NVDA and its pricing power.",
  macro_q="Analyze the impact of a 25bps rate cut on high-growth tech valuation.",
  quant_q="Project NVDA's price movement based on historical rate cut cycles."
)
```
